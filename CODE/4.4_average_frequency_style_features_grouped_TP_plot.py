...
# Load dataset
stylo_df_grouped = pd.read_excel("4.4_stylometrics_report_grouped_features_TP.xlsx")
...
data_melted['author'] = data_melted['author'].replace({'author_1': r'Verifying $\\mathcal{H}$', 'author_0': r'Not Verifying $\\mathcal{H}$'})

# Rename features for the plot
feature_mapping = {
    'Structural': r'$\mathbf{Structural}\boldsymbol{**}$',
    'NER': r'$\mathbf{NER}\boldsymbol{**}$',
    'Numbers': r'$\mathbf{Numbers}\boldsymbol{*}$'
}
...
hue_order = [r'Not Verifying $\\mathcal{H}$', r'Verifying $\\mathcal{H}$']
ax = sns.boxplot(
    data=data_melted,
    x='Feature',
    y='Normalized Value',
